...
# Travel Companion AI: результаты

Персональный планировщик путешествий в виде Telegram-бота.  
Агент на основе LLM (Mistral / OpenRouter) с живым поиском в интернете через DuckDuckGo.

## Возможности

- Пошаговый опрос: направление, даты, бюджет (кнопки), интересы.
- Живой поиск авиабилетов, отелей, достопримечательностей и ресторанов.
- Генерация структурированного плана с реальными ссылками.
- Разбивка длинных сообщений на части.
- Сохранение итоговых планов в SQLite.
- Управление секретами через `.env` и `.gitignore`.
- Базовая защита от перегрузок (rate limiting на стороне Telegram).
- Контентная модерация на уровне промпта системного сообщения.
- Обработка отказов LLM-провайдера с информативным сообщением пользователю.

## Стек

- **Язык:** Python 3.12
- **Фреймворк бота:** aiogram 3.x
- **LLM-провайдер:** Mistral AI API (OpenAI-совместимый SDK), поддержка OpenRouter
- **Оркестрация:** опционально LangGraph (для сложных сценариев)
- **Поиск:** DuckDuckGo (пакет `ddgs`)
- **Хранение:** SQLite (aiosqlite), в памяти — кэш диалогов
- **Секреты:** python-dotenv
